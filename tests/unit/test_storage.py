...
class TestDatabaseConceptManagerInit:
    """Test DatabaseConceptManager initialization."""
    ...
    def test_initialization(self, storage):
        ...
        manager = DatabaseConceptManager(storage)
        assert manager.storage == storage


class TestDatabaseConceptManagerOntologyMapping:
    """Test DatabaseConceptManager ontology name mapping."""
    ...
    def manager(self):
        ...
        storage = PostgreSQLStorage(config)
        return DatabaseConceptManager(storage)

    def test_engineering_ethics_domain_mapping(self, manager):
        """Test that engineering-ethics domain maps to correct ontologies."""
        ontologies = manager._get_ontology_names_for_domain('engineering-ethics')

        assert 'proethica-core' in ontologies
        assert 'proethica-intermediate' in ontologies
        assert 'engineering-ethics' in ontologies

    def test_unknown_domain_fallback(self, manager):
        """Test that unknown domains fall back to core ontologies."""
        ontologies = manager._get_ontology_names_for_domain('unknown-domain')

        # Should fall back to core ontologies
        assert 'proethica-core' in ontologies or len(ontologies) > 0


class TestDatabaseConceptManagerEmptyResponse:
    """Test DatabaseConceptManager empty response generation."""
    ...
    def test_empty_response_structure(self, manager):
        """Test that empty response has correct structure."""
        response = manager._empty_response('TestCategory', 'test-domain', 'approved')

        assert response['entities'] == []
        assert response['category'] == 'TestCategory'
        assert response['domain_id'] == 'test-domain'
        assert response['status'] == 'approved'
        assert response['total_count'] == 0
        assert response['source'] == 'ontology_entities_table'


class TestDatabaseConceptManagerGetEntities:
    """Test DatabaseConceptManager entity retrieval."""
    ...
    def test_get_entities_with_no_data(self, manager):
        """Test get_entities when no matching data exists."""
        result = manager.get_entities_by_category(
            'NonExistentCategory',
            'engineering-ethics'
        )
    ...
    def test_get_entities_entity_format(self, manager):
        """Test that returned entities have correct format."""
        # This test requires actual data - check format if entities exist
...
class TestDatabaseConceptManagerStubs:
    """Test DatabaseConceptManager stub methods."""
    ...
    def test_submit_candidate_concept_returns_error(self, manager):
        """Test that stub method returns appropriate error."""
        result = manager.submit_candidate_concept({}, 'test', 'user')
        assert 'error' in result

    def test_update_concept_status_returns_error(self, manager):
        """Test that stub method returns appropriate error."""
        result = manager.update_concept_status('id', 'approved', 'user')
        assert 'error' in result

    def test_get_candidate_concepts_returns_error(self, manager):
        """Test that stub method returns appropriate error."""
        result = manager.get_candidate_concepts('test')
        assert 'error' in result


class TestDatabaseConceptManagerDomainInfo:
    """Test DatabaseConceptManager domain info retrieval."""
    ...
    def test_get_domain_info_not_found(self, manager):
        """Test domain info for non-existent domain."""
        result = manager.get_domain_info('nonexistent-domain')

        assert 'status' in result
        # Should indicate not found or return empty
        assert result['status'] in ['not_found', 'error'] or result.get('error')

    def test_get_domain_info_response_structure(self, manager):
        """Test domain info response has expected structure."""
        result = manager.get_domain_info('engineering-ethics')

        # Should have domain_id at minimum
        assert 'domain_id' in result
        assert 'status' in result
