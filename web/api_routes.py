...
Provides REST API endpoints for ontology and version management, plus
the public SPARQL endpoint that honors the paper's claim that the
full 134-ontology graph is queryable at /sparql on the live domain.
"""
...
#
# The MCP server also exposes /sparql on its own port, but reviewers
# visiting https://ontserve.ontorealm.net/sparql (the web domain) expect
# a SPARQL endpoint there too. This route forwards to the shared
# SPARQLService instance initialized on the Flask app.
# ---------------------------------------------------------------------------
...
@api_bp.route('/sparql', methods=['GET', 'POST'])
def sparql_endpoint():
...
@api_bp.route('/sparql/status', methods=['GET'])
def sparql_status():
