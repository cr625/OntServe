...
# ---------------------------------------------------------------------------
# Cytoscape.js stylesheet (shared across all visualization endpoints)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Builder: enhanced visualization with Cytoscape.js config
# ---------------------------------------------------------------------------

def build_enhanced_visualization(
    ontology_name: str,
    include_reasoning: bool = True,
    layout_type: str = 'hierarchical',
    entity_limit: int = 1000,
) -> dict:
    """Build Cytoscape.js data with layout/style config and reasoning annotations.

    Returns a dict suitable for jsonify().
    """
    ontology, entities = _load_ontology_entities(ontology_name, limit=entity_limit)
    if ontology is None:
        return {'success': False, 'error': f'Ontology {ontology_name} not found'}
    ...
    for entity in entities:
        ...
        nodes.append({
            'data': node_data,
            'classes': (
                f"entity-node {entity.entity_type}"
                + (' inferred' if node_data.get('is_inferred') else ' explicit')
            ),
        })
        ...
        if entity.parent_uri:
            edges.append({
                'data': {
                    'id': f"{entity.uri}-subClassOf-{entity.parent_uri}",
                    'source': entity.uri,
                    'target': entity.parent_uri,
                    'type': 'subClassOf',
    ...
    layout = LAYOUT_OPTIONS.get(layout_type, LAYOUT_OPTIONS['hierarchical'])
    return {
        'success': True,
        'visualization': {
            'nodes': nodes,
            ...
            'style_options': CYTOSCAPE_STYLES,
        },
        'stats': {
